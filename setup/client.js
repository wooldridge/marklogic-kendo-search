...
const configDatabase = async (name, body, config) => {
  const options = {
    method: 'PUT',
...
const loadSearchOptions = async (config) => {
  ...
  if (config.rest.options.file.slice(-4) === "json") {
    options.headers = {
      'Content-Type': 'application/json'
    }
  }
...
// NOTE Not needed if database deleted with param forest-delete=data
...
// NOTE Not needed if database deleted
...
// NOTE Not needed if modules database deleted
